...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      sliderInput("opacity", HTML(paste(fa("wand-magic-sparkles"), "Map transparency:")), min = 0, max = 1, value = 0.6, step = 0.1),
...
server <- function(input, output, session) {
  ...
  observe({
    ...
    if (!is.null(bounds)) {
      ...
      # erddap_url <- paste0(
      #   "https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW.csv?", input$dataType, "%5B(", input$date, "T12:00:00Z)%5D%5B(",
      #   lat_range[1], "):(", lat_range[2], ")%5D%5B(",
      #   lon_range[1], "):(", lon_range[2],
      #   ")%5D"
      # )
      erddap_url <- paste0(
        "https://coastwatch.pfeg.noaa.gov/erddap/griddap/NOAA_DHW.csv?", input$dataType, "%5B(", input$date,
        "T12:00:00Z):1:(", input$date, "T12:00:00Z)%5D%5B(",
        lat_range[1], "):1:(", lat_range[2], ")%5D%5B(",
        lon_range[1] , "):1:(", lon_range[2],
        ")%5D"
      )
