...
tmp2 <- extract_ocean_data(dataset = "SST_NOAA_CRW_CoralTemp",
  space = c(70, 71, -2, -1),
  time = c("2004-09-01", "2004-09-10"),
  average="mean_annual",
  filename="Maldives.csv")
...
### NOAA ###
  ...
  ## EPSG:4326 is the format we want (good for NOAA CRW)
  ...
  if (askYesNo(paste0(
    "There are ", nrow(count.loop), " gridcells ", "(",
...
dimnames(dataset_var)[[3]] <- timeIdx.conv

tmp <-  as.data.frame.table(dataset_var) |>
...
dataset_array_extent <- terra::ext(min(LonIdx.conv), max(LonIdx.conv), min(LatIdx.conv), max(LatIdx.conv))
dataset_sp <- terra::rast(dataset_var, extent=dataset_array_extent, crs="EPSG:4326")
names(dataset_sp) <- timeIdx.conv
terra::time(dataset_sp) <- timeIdx.conv
terra::units(dataset_sp) <- this_var

    # output.df <- count.loop |>
    # dplyr::select(lon, lat, date) |>
    # dplyr::mutate(date = lubridate::ymd(as.Date(date))) |>
    # dplyr::mutate(xyz = as.numeric(dataset_var)) |>
    # dplyr::arrange(lon, lat) |>
    # dplyr::rename_with(.col = xyz, ~this_var) |>
    # dplyr::rename_with(.col = lon, ~"longitude") |>
    # dplyr::rename_with(.col = lat, ~"latitude")
#
#   ### average if selected
...
output.df.rast <- output.df |> dplyr::select(-date)

tmp <- sf::st_as_sf(output.df, coords = c("longitude", "latitude"), crs="EPSG:4326")

tmp2 <- terra::rast(tmp, subds="date")

    ### average if selected
#options(dplyr.summarise.inform = FALSE)
  if (summarise == "mean") {
    dataset_sp_summarised <- terra::tapp(dataset_sp, index = "years", fun = mean)
    names(dataset_sp_summarised) <- gsub("\\X","", names(dataset_sp_summarised))
  } else if (summarise == "mode") {
    dataset_sp_summarised
  } else if (summarise == "median") {
    dataset_sp_summarised
  } else if (summarise == "min") {
    dataset_sp_summarised
    ...
    dataset_sp_summarised <- dataset_sp
  }
