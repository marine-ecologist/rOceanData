...
extract_ocean_data(space = "tmpcoords.csv", time = c("2010-01-01", "2010-02-01"), filename = "temp", dataset="NWW3_Global_Best[shgt]")
load("temp.rda")

# spatial extent
extract_ocean_data(space = c(72, 74, -2, 6), time = c("2010-01-01", "2010-02-01"), filename = "maldivesspatialextent")
extract_ocean_data(space = c(72, 74, -2, 6), time = c("2010-01-01", "2010-02-01"), filename = "maldivesspatialextent", dataset = "ncdcOisst21Agg_LonPM180[sst]")
extract_ocean_data(space = c(-42, 40, 40, 42), time = c("2010-01-01", "2010-02-01"), filename = "maldivesspatialextent", dataset = 17)

# spatial points
extract_ocean_data(space = "limitedsites.xlsx", time = c("2010-01-01", "2010-02-01"), filename = "maldivesspatialpoints")
extract_ocean_data(space = "limitedsites.xlsx", time = c("2010-01-01", "2010-02-01"), filename = "maldivesspatialpoints", dataset = "ncdcOisst21Agg_LonPM180[sst]")
extract_ocean_data(space = "limitedsites.xlsx", time = c("2010-01-01", "2010-02-01"), filename = "maldivesspatialpoints", dataset = 17)



extract_ocean_data <- function(dataset = "none", filename = NULL, space = NULL, time = NULL, save.raster = TRUE) {

  #' @importFrom magrittr %>% # https://stackoverflow.com/questions/51124046/use-explicit-namespace-with-infix-operator
  ...
  get_rast <- function(grid, var) {
    ...
    r <- if (length(times) > 1) {
      ...
      terra::values(b) <- lazyeval::f_eval(~ var, this_dataset_nc$data)
    ...
    names(r) <- paste0(as.character(unique(lubridate::ymd(as.Date((unique(grid$data$time)))))))
     terra::time(r) <- as.Date(as.numeric(names(r)))
    r
  }
  ...
  ##### 1b.  select dataset from the list with prompt:
  ...
  # extract header information from dataset
    x <- info("dataset")
  ...
  ##### @@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@
  ##### 2. extract data
  ##### a. if spatial points (coordinate list)
  ...
  if (is.character(space)[1] == TRUE) {
    ...
    coordlist.rn <- convertlongitude(coordlist.rn)
    ...
    save(spatial_points_extract, file = paste0(filename, ".rda"))


  ##### b. if spatial extent (bounding box)
    ...
    if  (dataset.xmax>180) {
    longitude.extent.b <- c(360 + longitude.box[1], dataset.xmax)
    longitude.extent.a <- c(xmin, longitude.box[2])

    # get data for both extents with griddap with correct extent:
    datasetnc_b <- griddap("dataset", fmt = "nc", longitude = longitude.extent.b, latitude = latitude.extent, time = time)
    datasetnc_a <- griddap("dataset", fmt = "nc", longitude = longitude.extent.a, latitude = latitude.extent, time = time)

    # reconvert data output BACK from 0 to 360 from -180 to 180 and merge
    datasetnc_b.data <- datasetnc_b$data # get data.frame b
    datasetnc_b.data$longitude <- datasetnc_b.data$longitude - 360
    datasetnc_a.data <- datasetnc_a$data # get data.frame a
    datasetnc_data.merged <- rbind(datasetnc_a.data, datasetnc_b.data)  |> # merge dataframes
      arrange(longitude, latitude) |>
      select(longitude, latitude, sst, get(this_var)) #|> na.omit()
    ...
    #     } else {
    #
    #     save(assign(paste0(filename, 'spatial_extent_extract',), dataset_raster),
