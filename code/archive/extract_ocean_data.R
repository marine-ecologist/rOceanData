...
{ sourcedata <- base::as.data.frame(rbind(
  ...
  if (is.character(space)[1] == TRUE) {
    ...
    coordlist.rn <- coordlist |> # standardise longitude and latitude
    ...
    #get resolution
     tmp.res.nc <- rerddap::griddap(this_dataset, fmt = "nc", longitude = c(tmp.coord$longitude, (tmp.coord$longitude+1)), latitude =  c(tmp.coord$latitude, (tmp.coord$latitude+1)), time = time)
     tmp.res.longitude <- abs(tmp.res.nc$summary$dim$longitude$vals[2]-tmp.res.nc$summary$dim$longitude$vals[1])
     tmp.res.latitude <- abs(tmp.res.nc$summary$dim$latitude$vals[2]-tmp.res.nc$summary$dim$latitude$vals[1])
     tmp.res <- c(tmp.res.longitude, tmp.res.latitude)

    # first coordlist loop
    tmp.coord <- coordlist.rn[1, ]
    lon.pts <- c(tmp.coord$longitude, (tmp.coord$longitude))
    lat.pts <- c(tmp.coord$latitude, (tmp.coord$latitude))

    spatial.points.nc <- rerddap::griddap(this_dataset, fmt = "nc", longitude = lon.pts, latitude = lat.pts, time = time)
    #spatial.points.df.full <- spatial.points.nc$data
    spatial.points.raster <- get_raster(spatial.points.nc, this_var) # extract raster
    #    loop across layers of raster at a later stage
    #    pts.csv.raster[pts.csv.raster == pts.csv$summary$var[[this_var]]$missval] <- NA # replace missing values with NA
    names(spatial.points.raster) <- paste0("ts.", as.character(unique(lubridate::ymd(as.Date(spatial.points.nc$data$time)))))
    ...
    for (i in 1:nrow(coordlist.rn)) {
      ...
      tmp.spatial.points.raster <- get_raster(tmp.spatial.points.nc, this_var) # extract raster
      #     #pts.csv.raster.tmp[pts.csv.raster.tmp == pts.csv$summary$var[[this_var]]$missval] <- NA # replace missing values with NA
      #names(tmp.spatial.points.raster) <- paste0("ts.", as.character(unique(lubridate::ymd(as.Date(tmp.spatial.points.nc$data$time)))))
      coord.nc.list[[i]] <- tmp.spatial.points.raster
    }

      tmp <- raster::stack(coord.nc.list[[1]],coord.nc.list[[2]])
    raster::res(spatial.points.raster) <- tmp.res

    saveRDS(spatial.points.raster, file = paste0(filename, ".rds"))
    ...
    spatial.extent.raster <- get_raster(spatial.extent.nc, this_var) # extract raster
