...
# spatial extent
extract_ocean_data(filename = "junk/temp.csv", space = c(72, 74, -2, 6), time = c("2010-01-01", "2010-02-01"))
extract_ocean_data(dataset = "ncdcOisst21Agg_LonPM180[sst]", filename = "junk/temp.csv", space = c(72, 74, -2, 6), time = c("2010-01-01", "2010-02-01"))
extract_ocean_data(dataset = 17, filename = "junk/temp.csv", space = c(72, 74, -2, 6), time = c("2010-01-01", "2010-02-01"))

# spatial points
extract_ocean_data(filename = "junk/temp.csv", space = "limitedsites.xlsx", time = c("2010-01-01", "2010-02-01"))
extract_ocean_data(dataset = "ncdcOisst21Agg_LonPM180[sst]", filename = "junk/temp.csv", space = "limitedsites.xlsx", time = c("2010-01-01", "2010-02-01"))
extract_ocean_data(dataset = 17, filename = "junk/temp.csv", space = "limitedsites.xlsx", time = c("2010-01-01", "2010-02-01"))
...
extract_ocean_data <- function(dataset = "none", filename = NULL, space = NULL, time = NULL, save.raster = TRUE) {
  ...
  if (is.character(space)[1] == TRUE) {
    ...
    for (i in 1:nrow(coordlist.rn)) {
      ...
      lon.pts <- c(coordlist.rn$longitude, (coordlist.rn$longitude))
      lat.pts <- c(coordlist.rn$latitude, (coordlist.rn$latitude))
    ...
    coords.pts.output <- do.call(rbind, coords.pts)

    if (grepl(".*xlsx.*", space) == TRUE) {
      openxlsx::write.xlsx(coords.pts.output, filename)
    } else if (grepl(".*xls.*", space) == TRUE) {
      openxlsx::write.xlsx(coords.pts.output, filename)
    } else if (grepl(".*csv.*", space) == TRUE) {
      write.csv(coords.pts.output, filename)
    } else if (grepl(".*rda.*", space) == TRUE) {
      saveRDS(coords.pts.output, filename)
    } else {
      (stop(print("saved filename must be in one of the following formats: .xlsx, .xls, .csv, .rda")))
    }
    ...
    dataset_nc_box <- rerddap::griddap(this_dataset, fmt = "nc", longitude = longitude.box, latitude = latitude.box, time = time)
    dataset_raster <- get_raster(dataset_nc_box, ~ get(this_var)) # extract raster
    dataset_raster[dataset_raster == dataset_nc_box$summary$var[[this_var]]$missval] <- NA # replace missing values with NA
    names(dataset_raster) <- paste0("ts.", as.character(unique(lubridate::ymd(as.Date(dataset_nc_box$data$time)))))

    dataset.csv.data <- dataset_nc_box$data |> dplyr::select(longitude, latitude, time, this_var) # filter to just lon/lat/time/this_var columns
    dataset.csv.data[paste(this_var)][dataset.csv.data[paste(this_var)] == dataset_nc_box$summary$var[[this_var]]$missval] <- NA # replace missing values with NA
    dataset.csv.data[dataset.csv.data == 0] <- NA # replace 0 with NA (check later needed?)



    if (save.raster == TRUE) {
      save(dataset.csv.data, dataset_raster, file=paste0(sub("\\.[^.]*$", "", filename), ".rda"))
    if (grepl(".*csv.*", filename) == TRUE) {
      write.csv(dataset.csv.data, filename)
      save(dataset.csv.data, dataset_raster, file=paste0(sub("\\.[^.]*$", "", filename), ".rda"))
    } else if (grepl(".*rda.*", filename) == TRUE) {
     save(dataset.csv.data, dataset_raster, file=paste0(sub("\\.[^.]*$", "", filename), ".rda"))
    } else {
      (stop(print("saved filename must be in one of the following formats: .csv, .rda")))
    }
    ...
    if (grepl(".*csv.*", filename) == TRUE) {
      ...
      save(dataset.csv.data, file=paste0(sub("\\.[^.]*$", "", filename), ".rda"))
    } else {
      (stop(print("saved filename must be in one of the following formats: .csv, .rda")))
    }
