...
extract_ocean_data <- function(dataset = "none", filename = NULL, space = NULL, time = NULL, save.raster = TRUE) {
  ...
  if (is.character(space)[1] == TRUE) {
    ...
    spatial.points.raster <- get_raster(spatial.points.nc, ~ get(this_var)) # extract raster
    ...
    for (i in 2:nrow(coordlist.rn)) {
      ...
      tmp.spatial.points.raster <- get_raster(tmp.spatial.points.nc, ~ get(this_var)) # extract raster
      ...
      spatial.points.raster <- tmp.spatial.points.raster
    }

    save(spatial.points.raster, spatial.points.nc, file = paste0(filename, ".rda"))
